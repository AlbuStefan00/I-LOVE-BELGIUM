...
function createBubbles(){
  document.querySelectorAll(".bubbles").forEach(c=>{
    for(let i=0;i<20;i++){
...
const cities=[
  ...
  { name:"Charleroi", coords:[50.4114,4.4448], emoji:"🏭", text:"Industrial city", img:"https://upload.wikimedia.org/wikipedia/commons/7/72/Charleroi_Belfry.jpg" },
  { name:"Ostend", coords:[51.2167,2.9], emoji:"🏖️", text:"Seaside resort", img:"https://upload.wikimedia.org/wikipedia/commons/0/0d/Oostende_Beach.jpg" },
  { name:"Mechelen", coords:[51.0256,4.4777], emoji:"⛪", text:"St. Rumbold's Cathedral", img:"https://upload.wikimedia.org/wikipedia/commons/3/39/Mechelen_Rumbold.jpg" },
  { name:"Hasselt", coords:[50.9333,5.3333], emoji:"🌸", text:"Japanese Garden & jenever", img:"https://upload.wikimedia.org/wikipedia/commons/d/d4/Hasselt_City_Hall.jpg" }
];
...
const brusselsDetails=[
  ...
  { name:"Magritte Museum", coords:[50.842,4.36], emoji:"🖼️", text:"René Magritte art museum", img:"https://upload.wikimedia.org/wikipedia/commons/9/92/Magritte_Museum.jpg" },
  ...
  { name:"Comic Strip Center", coords:[50.8507,4.3603], emoji:"📚", text:"Tintin & comics", img:"https://upload.wikimedia.org/wikipedia/commons/e/e0/Comic_Strip_Center_Brussels.jpg" },
...
function addPoints(points,sectionTitle){
  ...
  points.forEach(p=>{
    ...
    item.addEventListener("click",()=>{ map.setView(p.coords,13); marker.openPopup(); });
...
function checkAnswer(s,c){
  document.getElementById("feedback").textContent=(s===c)?"✅ Correct!":"❌ Wrong! Correct: "+c;
...
function wireEnlarge(){
  document.querySelectorAll("img.enlarge").forEach(img=>{
    img.addEventListener("click",()=>{
      const f=document.getElementById("fullscreenImg");
      f.style.display="flex";
      f.querySelector("img").src=img.src;
    });
  });
}
function closeFullscreen(){ document.getElementById("fullscreenImg").style.display="none"; }
wireEnlarge();
